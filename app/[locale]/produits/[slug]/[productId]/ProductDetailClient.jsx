...
export default function ProductDetailPage() {
  ...
  // Nombre de colonnes dynamiques (1, 2, 3...) → n'appliqué qu'à partir de md
      ...
      <main className="bg-slate-50 min-h-screen relative overflow-x-hidden">
        ...
        <div className="w-full mx-auto max-w-7xl px-4 pt-8 pb-24">
          ...
          <section className="mt-10">
            {loading ? (
              /* --------- SKELETON : 1 carte/ligne en mobile, md = colonnes dynamiques --------- */
              <div
                className="grid gap-6 grid-cols-1 md:[grid-template-columns:repeat(var(--cols),minmax(0,1fr))]"
                style={{ "--cols": String(Math.max(2, cols)) }}
              >
              ...
              /* --------- GRILLE LOADED : 1 carte/ligne en mobile, md = colonnes dynamiques --------- */
              <motion.div
                initial="hidden"
                whileInView="show"
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            ...
            <button
              onClick={() => setLightbox(false)}
              className="absolute right-4 top-4 z-[200] h-10 w-10 rounded-full bg-white/90 text-[#0B2239] shadow grid place-items-center pointer-events-auto"
              aria-label="Fermer"
            >
              ✕
            </button>
            ...
            {imagesRaw.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 z-[200] rounded-full bg-white/90 px-3 py-2 text-[#0B2239] shadow pointer-events-auto"
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 z-[200] rounded-full bg-white/90 px-3 py-2 text-[#0B2239] shadow pointer-events-auto"
                  aria-label="Suivant"
                >
                  ›
                </button>
            ...
            {/* image affichée */}
            <div className="absolute inset-0 p-10 pointer-events-none" onClick={(e) => e.stopPropagation()}>
              <Image
