...
function pickName(cat, locale) {
  return ((cat?.translations && (cat.translations[locale] || cat.translations.fr || cat.translations.en)) || cat?.label || "");
}
function slugifySafe(s = "") {
  return String(s).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)+/g, "");
}
const makeCatHref = (cat, locale) => `/${locale}/produits/${cat?.slug || slugifySafe(pickName(cat, locale))}`;
function pickProdName(p, locale) {
  const byLocale = locale === "en" ? (p?.name_en || p?.label_en || p?.title_en) : (p?.name_fr || p?.label_fr || p?.title_fr);
  return byLocale || (p?.translations && (p.translations[locale] || p.translations.fr || p.translations.en)) || p?.name || p?.label || p?.title || "";
}
...
function swapLocaleInPath(path, nextLocale) {
  ...
  else base = base === "/" ? `/${nextLocale}` : `/${nextLocale}${base}`;
  return q ? `${base}?${q}` : base;
}
const PATH_LOCALE_RE = /^\/(fr|en)(?:\/|$)/;

/* ======================= Component ======================= */
export default function SiteHeader({ mode = "public" }) {
  ...
  useEffect(() => {
    let desired = "fr";
    try { const saved = localStorage.getItem("mtr_locale"); if (saved === "en" || saved === "fr") desired = saved; } catch {}
    const m = PATH_LOCALE_RE.exec(pathname);
    const pathLocale = m?.[1] || null;

    if (!localStorage.getItem("mtr_locale") && (pathLocale === "fr" || pathLocale === "en")) desired = pathLocale;

    setLocale(desired);
    if (typeof document !== "undefined") document.documentElement.lang = desired;

    if (pathLocale !== desired) {
      const qs = sp?.toString();
      const nextUrl = qs ? `${swapLocaleInPath(pathname, desired)}?${qs}` : swapLocaleInPath(pathname, desired);
  ...
  const isLoggedClient = mode === "client" || (isAuthenticated && role === "client");
  ...
  /* switch langue (préserve la query) */
  ...
  const ProductsMenu = ({ cats, locale }) => {
    ...
    cats.forEach((c) => {
      ...
      if (!pid) return;
      const arr = childrenMap.get(pid) || []; arr.push(c); childrenMap.set(pid, arr);
    });
    ...
    return (
      ...
      >
        <button
          type="button"
          className="group relative px-3 py-2 text-[15px] md:text-[16px] font-medium text-[#0B2239] hover:text-[#F5B301]"
          aria-haspopup="true"
        ...
        {menuOpen && (
          <div className="absolute left-0 top-full z-50 before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-2">
            {/* Colonne gauche */}
            <ul className="relative w-60 sm:w-64 rounded-lg bg-white p-2 shadow-2xl ring-1 ring-slate-200 after:content-[''] after:absolute after:top-0 after:right-[-8px] after:w-2 after:h-full">
              {tops.map((parent) => {
                ...
                return (
                  <li key={id}>
                    <Link
                      ...
                      className={`flex items-center justify-between rounded-md px-3.5 py-3 text-[15px] transition ${
            ...
            {/* Colonne droite */}
            {hoveredParent && (
              <div className="absolute left-[100%] top-0 ml-2 w-68 sm:w-72 rounded-lg bg-white p-2 shadow-2xl ring-1 ring-slate-200">
                {Array.isArray(childrenMap.get(hoveredParent)) && childrenMap.get(hoveredParent).length > 0 && (
                  <ul className="mb-1">
                    {(childrenMap.get(hoveredParent) || []).map((child) => (
                      <li key={child?._id || pickName(child, locale)}>
                        <Link
                          href={makeCatHref(child, locale)}
                          className="block rounded-md px-3.5 py-2 text-[14px] sm:text-[15px] text-[#0B2239] hover:bg-slate-50"
                        >
                ...
                {(() => {
                  ...
                  return showProductsPanel ? (
                    <div className="mt-1 border-t border-slate-100 pt-2">
                      <div className="px-3.5 pb-1 text-[11px] sm:text-xs uppercase tracking-wide text-slate-400">
                        {t("nav.products")}
                      </div>
                      <ul>
                        {prods.map((p) => (
  ...
  const ClientNavItemsDesktop = () => {
    ...
    const itemCls = "px-3 py-2 text-[15px] md:text-[16px] font-medium text-[#0B2239] hover:text-[#F5B301]";
    return (
      <>
        ...
        <div ref={ref} className="relative">
          ...
          {servicesOpen && (
            <div role="menu" className="absolute left-0 top-full mt-1 w-64 rounded-lg border border-slate-200 bg-white shadow-lg z-50">
              <Link href={`/${locale}/client/mes-devis`} role="menuitem" className="block px-4 py-2 text-[15px] text-slate-700 hover:bg-slate-50">{t("client.myQuotes")}</Link>
              <Link href={`/${locale}/client/mes-reclamations`} role="menuitem" className="block px-4 py-2 text-[15px] text-slate-700 hover:bg-slate-50">{t("client.myClaims")}</Link>
            </div>
          )}
        </div>
  ...
  const UserMenu = () => {
    ...
    return (
      <div ref={ref} className="relative">
        <button type="button" onClick={() => setUOpen((s) => !s)} aria-label={t("userMenu.aria")} className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-slate-200 text-[#0B2239] hover:bg-slate-50">
        ...
        {uOpen && (
          <div className="absolute right-0 top-full mt-2 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl z-50">
            ...
            <button
              onClick={async () => { setUOpen(false); await logout(); router.replace(`/${locale}`); }}
    ...
    <header className="sticky top-0 z-40">
      ...
      <div className="bg-[#0B2239] text-white">
        <div className="mx-auto max-w-screen-2xl px-2 sm:px-4">
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 min-h-[40px] py-1 text-[12px] sm:text-[14px]">
            {/* Left links */}
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                <button onClick={() => switchLang("fr")} className={`${locale === "fr" ? "ring-2 ring-[#F5B301] rounded-full" : ""} px-2 py-1 bg-transparent border-0 text-[13px] sm:text-[14px] font-semibold`} title={t("aria.langFR")} aria-pressed={locale === "fr"}>FR</button>
                <button onClick={() => switchLang("en")} className={`${locale === "en" ? "ring-2 ring-[#F5B301] rounded-full" : ""} px-2 py-1 bg-transparent border-0 text-[13px] sm:text-[14px] font-semibold`} title={t("aria.langEN")} aria-pressed={locale === "en"}>EN</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto max-w-screen-2xl px-3 sm:px-6">
          <div className="flex h-14 sm:h-16 items-center justify-between">
            {/* Logo */}
            <Link href={`/${locale}`} className="flex items-center gap-2 sm:gap-3" aria-label={t("logoAlt")}>
              <img
                src="/logo_MTR.png"
                alt={t("logoAlt")}
            ...
            <RightActions locale={locale} isLoggedClient={isLoggedClient} />
          </div>
        </div>
  ...
  /* ===== helpers render ===== */
  function RightActions({ locale, isLoggedClient }) {
    return (
  ...
  function MobileDrawer({ open, setOpen, isLoggedClient, isHome, t, locale, goToSection }) {
    if (!open) return null;
    return (
