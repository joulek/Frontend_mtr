  ...
  async function handleLogout() {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("mtr:auth", { detail: { state: "logout" } })
      );
    } catch {}
    ...
    <header className="font-inter sticky top-0 z-40">
      ...
      <div className="bg-[#0B2239] text-white">
        <div className="mx-auto max-w-screen-2xl px-3 sm:px-4">
          {/* sous 640px: colonne/2 lignes — au-delà: ligne وحدة */}
          <div className="flex flex-col sm:flex-row sm:h-10 py-2 sm:py-0 items-center sm:items-stretch justify-between gap-2">
            {/* nav gauche */}
            <nav className="flex flex-wrap items-center justify-center sm:justify-start gap-x-3 gap-y-1 text-[12px] sm:text-[14px]">
              <button
                type="button"
            ...
            {/* actions droite */}
            <div className="flex items-center justify-center sm:justify-end w-full sm:w-auto gap-2 sm:gap-3">
              {/* cache icon fi xs باش ما يزحّمش */}
              <a
                href="https://www.facebook.com/profile.php?id=100076355199317&locale=fr_FR"
                target="_blank"
                rel="noreferrer"
                className="hidden sm:inline-flex rounded-full bg-white/10 p-1.5 hover:bg-white/20"
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`h-7 px-2 rounded-full text-[12px] font-semibold ${
                    locale === "fr" ? "ring-2 ring-[#F5B301]" : "ring-0"
                  }`}
                  title={t("aria.langFR")}
                ...
                <button
                  ...
                  className={`h-7 px-2 rounded-full text-[12px] font-semibold ${
                    locale === "en" ? "ring-2 ring-[#F5B301]" : "ring-0"
                  }`}
                  title={t("aria.langEN")}
